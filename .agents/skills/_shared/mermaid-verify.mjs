...
function findChrome() {
  const glob1 = path.join(os.homedir(), '.puppeteer-cache', 'chrome');
  if (fs.existsSync(glob1)) {
    for (const versionDir of fs.readdirSync(glob1)) {
      const candidates = [
        path.join(glob1, versionDir, 'chrome-mac-arm64', 'Google Chrome for Testing.app', 'Contents', 'MacOS', 'Google Chrome for Testing'),
        path.join(glob1, versionDir, 'chrome-linux64', 'chrome'),
        path.join(glob1, versionDir, 'chrome-win64', 'chrome.exe'),
      ];
  ...
  const glob2 = path.join(os.homedir(), '.cache', 'puppeteer', 'chrome-headless-shell');
  if (fs.existsSync(glob2)) {
    for (const versionDir of fs.readdirSync(glob2)) {
      const candidates = [
        path.join(glob2, versionDir, 'chrome-headless-shell-win64', 'chrome-headless-shell.exe'),
        path.join(glob2, versionDir, 'chrome-headless-shell-linux64', 'chrome-headless-shell'),
        path.join(glob2, versionDir, 'chrome-headless-shell-mac-arm64', 'chrome-headless-shell'),
      ];
      for (const cand of candidates) {
        if (fs.existsSync(cand)) return cand;
      }
...
if (!CHROME) {
  console.error('⚠️  Không tìm thấy Chrome / chrome-headless-shell — mmdc sẽ fail khi launch.');
  console.error('   Cài qua: npx puppeteer browsers install chrome-headless-shell');
  process.exit(2);
}
