...
description: Vẽ sơ đồ quy trình nghiệp vụ đa vai trò (Activity Swimlane) bằng PlantUML, phân tách cột (lane) rõ ràng, tự động render SVG qua PlantUML server và nhúng vào tài liệu.
---

### Cách gọi
Kích hoạt hoặc yêu cầu vẽ sơ đồ bằng ngôn ngữ tự nhiên, ví dụ:
- `@activity-swimlane `
- Hoặc yêu cầu: "vẽ sơ đồ activity-swimlane cho..."
...
- **Render qua plantuml.com** (`render.py`) — máy này KHÔNG có Java. **TRADE-OFF**: nội dung diagram (tên lane/bước) gửi qua internet mỗi lần render. Nội dung nhạy cảm → không dùng skill này (xem Gotchas).
- **AI viết source .puml, KHÔNG tự tính toạ độ** — PlantUML layout engine lo. Vai trò AI = mô tả đúng nghiệp vụ (lane/bước/nhánh/loop).
- **Compile phải PASS** trước khi báo xong. `render.py` bắt cả lỗi HTTP lẫn "Syntax Error" nhúng trong SVG. Fail → sửa source, render lại (tối đa 2 lần).
...
6. L1 plan preview (prose BA-friendly: N bước, M decision, K lane). User Y → tiếp.
   ▼
7. Write .puml → render.py --png → SVG + PNG.
...
```bash
.agents/skills/activity-swimlane/render.py docs/{feature}/srs/{feature}-{slug}-swimlane.puml --png
...
> Nguồn PlantUML: `{feature}-{slug}-swimlane.puml`. Sửa .puml → chạy `render.py` regen .svg.
```
...
- **Syntax Error nhúng trong SVG** — plantuml.com trả HTTP 200 kèm ảnh chứa text lỗi. `render.py` đã grep bắt "Syntax Error" → exit non-0. Đừng bỏ qua.
...
- **Path ảnh trong flows.md** — viết tương đối với vị trí flows.md (ở `srs/`), tức `![...](./{feature}-{slug}-swimlane.svg)` (cùng thư mục). `/preview` và `/export` TỰ ĐỘNG xử lý ảnh này:
  ...
  - **Nên GIỮ file `.png` cạnh `.svg`** (render.py --png sinh cả 2) — export PDF/DOCX dùng lại PNG này, khỏi convert lại.
...
- [example-movie-booking.puml](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/skills/activity-swimlane/references/example-movie-booking.puml) (mẫu chuẩn đã verify: 2 lane, 3 loop retry dùng `repeat while` + `backward:`, mọi nhánh tới stop — tham chiếu cách viết loop có bước xử lý giữa chừng)
