---
name: brainstorm
description: Ghi lại ý tưởng BA thô và phỏng vấn từng nhóm để làm rõ luồng, quyết định, số liệu và wording trước khi viết URD hoặc PRD; dùng ngôn ngữ nghiệp vụ và yêu cầu xem trước kế hoạch trước khi ghi tài liệu.
---

### Cách gọi
Kích hoạt hoặc yêu cầu brainstorm bằng ngôn ngữ tự nhiên, ví dụ:
- `/brainstorm <mô tả ý tưởng>`
- `/brainstorm @<đường dẫn file>`
- `/brainstorm` (để phỏng vấn tương tác)
...
7. **Auto-fill sections** theo `templates/brainstorm.md` (13 sections):
...
10. **Write** `docs/{feature}/brainstorms/{idea-slug}.md` từ `templates/brainstorm.md`.
11. **Set env trước Write** (hook ghi activity.log): `ANTIGRAVITY_SKILL_NAME=/brainstorm` + `ANTIGRAVITY_CHANGELOG_AUTHOR={@author}` + `ANTIGRAVITY_CHANGELOG_NOTE` (≤80 ký tự, vd `initial brainstorm scaffold cho {title}, deep mode, {N} OQs`). Hook ghép cả dòng — skill KHÔNG tự viết `{date} | ... | {note}`.
...
13. **Phase E — Resolve Open Questions (PRIORITY gate trước downstream)** — per `rules/resolve-oqs.md`. Brainstorm là gốc → chỉ có own OQs (Mục 12), không inherit. Collect → prompt Y/skip/ids → loop 1-by-1 → side-effect L2 diff cho Assumptions/Risks/Capabilities nếu OQ tác động → activity log (hook).
...
13b. **Phase E2 — Mark "đã chi tiết hóa" ngược lên PRD sản phẩm** — sau khi brainstorm write xong:
    ...
    - **Tìm thấy row** + cột Chi tiết hóa chưa phải `✅` → đếm `docs/{feature}/brainstorms/*.md` (N file) → **propose L2 diff** đổi cột Chi tiết hóa của row đó sang `✅ đã chi tiết (N brainstorm)`. User Y → Edit brief + set env `ANTIGRAVITY_CHANGELOG_NOTE=mark {feature} đã chi tiết hóa ({N} brainstorm)` (+ SKILL_NAME/AUTHOR) trước Edit; hook ghi activity.log.
...
- [keyword-detection.md](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/rules/keyword-detection.md)
- [resolve-oqs.md](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/rules/resolve-oqs.md)
- [changelog.md](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/rules/changelog.md)
- [brainstorm.md (Template)](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/skills/brainstorm/templates/brainstorm.md)
- [example-brainstorm.md (Ví dụ)](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/skills/brainstorm/references/example-brainstorm.md)
