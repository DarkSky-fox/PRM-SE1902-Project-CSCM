---
name: d2-activity
description: Vẽ sơ đồ Activity đẹp standalone bằng công cụ D2 (layout ELK, đường vẽ vuông góc không chồng chéo) từ mô tả quy trình nghiệp vụ để export hoặc trình bày cho stakeholder.
---

### Cách gọi
Kích hoạt hoặc yêu cầu vẽ sơ đồ bằng ngôn ngữ tự nhiên, ví dụ:
- `@d2-activity \"<mô tả quy trình>\" [--feature <slug>]`
- Hoặc yêu cầu: "vẽ sơ đồ d2-activity cho..."
# d2-activity — Activity / Flowchart Diagram (D2, layout ELK)

> Họ skill D2: `/d2-activity` (flow này) · `/d2-erd` (data model) · `/d2-architect` (kiến trúc hệ thống). Cả 3 dùng chung `render.py` ở `.agents/skills/d2-activity/`.
...
- **Output cố định** `docs/{feature}/d2/{slug}.d2` + `.svg`. KHÔNG ghi vào `flows.md`. KHÔNG flag layout/theme/direction (đã chốt ELK + theme neutral trong `render.py`).
...
- **Render qua `render.py`** — KHÔNG gọi `d2`/Chrome trực tiếp trong skill (script lo path + flag).
...
5. L1 plan preview (prose BA-friendly: N bước, M nhánh, K lane). User Y → tiếp
   ▼
6. Write {slug}.d2 → chạy render.py → sinh {slug}.svg
...
```bash
.agents/skills/d2-activity/render.py docs/{feature}/d2/{slug}.d2
