---
name: d2-architect
description: Vẽ sơ đồ kiến trúc hệ thống (Architecture Diagram) dạng container lồng nhau bằng D2 để thể hiện trực quan bối cảnh kỹ thuật, dịch vụ và liên kết hệ thống.
---

### Cách gọi
Kích hoạt hoặc yêu cầu vẽ sơ đồ bằng ngôn ngữ tự nhiên, ví dụ:
- `@d2-architect [--feature <slug> | \"<mô tả hệ thống>\"]`
- Hoặc yêu cầu: "vẽ sơ đồ d2-architect cho..."
# d2-architect — System / Architecture Diagram (D2, nested container + ELK)

> Họ skill D2: `/d2-activity` (flow) · `/d2-erd` (data model) · `/d2-architect` (kiến trúc này). Cả 3 dùng chung `render.py` ở `.agents/skills/d2-activity/`.
...
- **Render qua `.agents/skills/d2-activity/render.py`** (dùng chung). KHÔNG gọi d2/Chrome trực tiếp.
...
4. L1 plan preview (prose: K khối, N service, M dịch vụ ngoài). User Y → tiếp
   ▼
5. Write {slug}.d2 → render.py → {slug}.svg (compile fail → sửa, tối đa 2 lần)
   ▼
...
```bash
.agents/skills/d2-activity/render.py docs/{feature}/d2-architect/{slug}.d2
