...
- `@sequence \"<description>\" [--feature <slug>]`
- Hoặc yêu cầu: "vẽ sơ đồ sequence cho..."
...
9.7. **Tự review diagram (CHỈ khi vượt ngưỡng phức tạp)** — nếu fact-list ở bước 2.5 có **≥3 Alternative/Error Flow**, HOẶC diagram có **≥4 participant**, HOẶC **nesting alt/opt ≥2 tầng**, HOẶC có **callback/timeout/webhook**, tự đối chiếu và chấm điểm diagram dựa trên **Tiêu chí tự review diagram** bên dưới. Đảm bảo verdict đạt `approve` và khắc phục mọi lỗi BLOCKING/WARNING trước khi hoàn tất. Đo theo **tổng độ phức tạp** — số actor đơn thuần không đủ (flow 3-actor thẳng thì đơn giản, flow 2-actor 18-message 3-nhánh-lồng lại phức tạp). Dưới mọi ngưỡng trên, bước 9.6 tự-đối-chiếu (không agent) đã đủ — SKIP 9.7, đi thẳng bước 10 để tránh overhead cho case đơn giản.
...
- [example-sequence.md](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/skills/sequence/references/example-sequence.md)
