---
name: state
description: Vẽ sơ đồ chuyển trạng thái (State Diagram) bằng Mermaid thể hiện các trạng thái của thực thể, điều kiện chuyển đổi và trigger nhằm kiểm soát toàn bộ vòng đời đối tượng.
---

### Cách gọi
Kích hoạt hoặc yêu cầu vẽ sơ đồ bằng ngôn ngữ tự nhiên, ví dụ:
- `@state <entity> [--feature <slug>]`
- Hoặc yêu cầu: "vẽ sơ đồ state cho..."
...
- **Per [diagram-selection.md](file:///C:/Code/PRM/PRM-SE1902-Project-CSCM/.agents/rules/diagram-selection.md)** — entity ≥3 states trước proceed; <3 → warn "bảng đủ, có cần diagram?".
